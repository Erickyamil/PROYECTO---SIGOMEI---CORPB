-- ─────────────────────────────────────────────────────────────────────────
-- SIGOMEI TEST — DDL MySQL 8.0 (Entorno de pruebas)
-- Base de datos: sigomei_test
-- ─────────────────────────────────────────────────────────────────────────

SET FOREIGN_KEY_CHECKS = 0;

CREATE DATABASE IF NOT EXISTS sigomei_test
...
CREATE TABLE orden_mantenimiento (
    ...
    CONSTRAINT uq_odm_equipo_fecha
        UNIQUE (id_equipo, fecha_programada),

    CONSTRAINT fk_odm_equipo
        FOREIGN KEY (id_equipo)
        REFERENCES equipo(id_equipo)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,

    CONSTRAINT fk_odm_tecnico
        FOREIGN KEY (id_tecnico)
        REFERENCES tecnico(id_tecnico)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,

    CONSTRAINT fk_odm_usuario
        FOREIGN KEY (creado_por)
        REFERENCES usuario(id_usuario)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,

    CONSTRAINT ck_costo_real
...
CREATE INDEX idx_odm_equipo_fecha_estado
    ON orden_mantenimiento (id_equipo, fecha_programada, estado);
...
CREATE TABLE nota_seguimiento (
    ...
    CONSTRAINT fk_nota_odm
        FOREIGN KEY (id_odm)
        REFERENCES orden_mantenimiento(id_odm)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,

    CONSTRAINT fk_nota_user
...
CREATE INDEX idx_hist_entidad
    ON historial_estado (entidad_tipo, entidad_id);

CREATE INDEX idx_hist_fecha
    ON historial_estado (fecha_cambio);
...
INSERT INTO usuario (id_usuario, correo, password_hash, rol)
VALUES
(
    'u-0001-admin',
    'admin@sigomei.mx',
    '$2b$12$cARvgmX6BVAkYvRFdfKv8Ovii9hCx8bztJPQWd0j3SlSrz3SAUr7q',
    'Administrador'
),
(
    'u-0002-super',
    'super@sigomei.mx',
    '$2b$12$60klIvEpO8oNnwXXUdgABOyb1tdQFI4eXjU8vO84Oh67G3oc8Pwka',
    'Supervisor'
),
(
    'u-0003-super',
    'super2@sigomei.mx',
    '$2b$12$SL0ofEZUaJLrNejwJF427eL6t3FVGoJLKPy2X2gydfwY2yIO/7tlO',
    'Supervisor'
);
